@props([
  ...
  'fullWidth'   => false,  // span full container width?
])
...
>
  ...
  >
    <div class="flex flex-wrap gap-1 overflow-hidden">
      ...
      <template x-for="id in {{ $selected }}" :key="id">
        <span
          class="flex items-center max-w-full text-xs font-medium bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full"
        >
          <img
          ...
          <button class="ml-1 text-gray-500" @click.stop="{{ $selected }}.splice({{ $selected }}.indexOf(id),1)">×</button>
        </span>
      </template>
    </div>
  ...
  {{-- we still need to post the selected values --}}
  <template x-for="id in {{ $selected }}" :key="id">
