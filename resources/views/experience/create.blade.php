<x-layout title="Add Experience">
  <x-slot name="heading">Add Experience</x-slot>
    ...
    <div class="max-w-3xl w-full bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
      ...
      >
        ...
        <div class="space-y-4">
          ...
          <button type="button" @click="addAchievement()" class="text-sm text-blue-600 hover:underline">+ Add Achievement</button>
        </div>
        ...
        <div class="flex justify-between space-x-4 pt-6 border-t border-gray-200 dark:border-gray-700">
          <form method="POST" action="{{ route('experience.destroy', $experience ?? 0) }}" onsubmit="return confirm('Are you sure?')">
            @csrf
            @method('DELETE')
  ...
  <script>
    function experienceForm() {
      return {
        ...
        company:      {!! json_encode(old('company','')) !!},
